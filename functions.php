<?php
/**
 * Ostentus functions and definitions.
 *
...
if ( ! function_exists( 'ostentus_setup' ) ) :
/**
 ...
 */
function ostentus_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on Ostentus, use a find and replace
	 * to change 'ostentus' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'ostentus', get_template_directory() . '/languages' );
	...
	// add_theme_support( 'custom-background', apply_filters( 'ostentus_custom_background_args', array(
	// 	'default-color' => 'ffffff',
	// 	'default-image' => '',
	// ) ) );
}
endif; // ostentus_setup
add_action( 'after_setup_theme', 'ostentus_setup' );
 ...
 */
function ostentus_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'ostentus_content_width', 640 );
}
add_action( 'after_setup_theme', 'ostentus_content_width', 0 );
 ...
 */
function ostentus_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'ostentus' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
function ostentus_scripts() {
	...
	wp_enqueue_script( 'ostentus-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

    wp_enqueue_script( 'ostentus-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery' ), '20150302', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'ostentus_scripts' );
